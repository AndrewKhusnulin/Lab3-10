...
boost::mutex mtx;//используем для контроля доступа к объектам, к которым обращаются из разных потоков
...
void acceptorLoop();//Обрабатывает новые клиентские соединения TCP / IP
void requestLoop();//Обрабатывает клиентские запросы, добавляя их в MessageQueue
void responseLoop();//Обрабатывает ответы сервера на клиенты, читает из MessageQueue
...
int main(int argc, char** argv)
{
	...
	boost::this_thread::sleep(boost::posix_time::millisec(sleepLen::sml)); //posix_time позволяет не привязывать время к дате
	...
	threads.join_all();//Данный поток ждет все остальные
...
void acceptorLoop()//Ожидает, что объект io_service получит новое клиентское соединение, когда это произойдет, мы создаем новый сокет TCP / IP и добавим его в наш список клиентов.
{
...
void disconnectClient(socket_ptr clientSock)
{
	...
	clientSock->shutdown(tcp::socket::shutdown_both);//отключаем отправку и прием по сокету
	clientSock->close();
...
void responseLoop()//отправляем строку чата из messageQueue всем получателям в списке клиентов
{
	for (;;)
	{
